...
import { recuperarSenhaSchema, RecuperarSenhaSchemaType } from "../../security/validations/RecuperarSenhaSchema";
...
export const useRecuperarSenha = () => {
  ...
  const onSubmit = async (data: RecuperarSenhaSchemaType) => {
    ...
    try {
      const resultado = await recuperarSenha('/usuarios/recuperarsenha', { usuario: data.usuario });
